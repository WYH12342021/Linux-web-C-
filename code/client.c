...
void* client(void* arg){
	CA* ca = (CA*)arg;
	printf("[%s][Line: %d][%s]: ca->conn=%d\n", __FILE__, __LINE__, __func__,ca->conn);
	printf("%d.%ld > 客户机线程处理开始\n",getpid(),syscall(SYS_gettid));
	for(;;){
		printf("%d.%ld > 接受请求\n",getpid(),syscall(SYS_gettid));
		char* req = recvRequest(ca->conn);	//recvRequest定义在socket.c里
		if(req == NULL){
			break;
		}
		printf("%d.%ld > 请求电文:%s\n",getpid(),syscall(SYS_gettid),req);	//打印客户端请求
		...
		//处理路径
		//资源路径	/home/wang/Project/Step2/project/home
		...
		if(searchResource(path) == -1){
			///home/wang/Project/Step2/project/home/404.html
			hres.status  = 404;
