...
int acceptClient(void){
	...
	int conn = accept(s_sock,(struct sockaddr*)&cli,&len);	//TCP链接为每个客户端生成一个套接字conn
	if(conn == -1){
		perror("accept");
		return -1;
	}
	printf("%d.%ld > 已经接受到%s.%hu的连接\n",getpid(),syscall(SYS_gettid),
				inet_ntoa(cli.sin_addr),ntohs(cli.sin_port));		//客户端的IP和端口号
	return conn;
}
...
char* recvRequest(int conn){
	...
	for(;;){
		...
		if(byte == 0){
			printf("%d.%ld > 客户端关闭了连接\n",getpid(),syscall(SYS_gettid));
			break;
		}
		//扩大存储区
		req = realloc(req,len+byte+1);
		//将此次接收到的数据，拷贝到扩大出来的空间
